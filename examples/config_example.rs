...
"#;
    ...
    let yaml_config = ConfigBuilder::new()
...
"#;
    ...
    match ConfigBuilder::new().from_yaml(invalid_yaml).and_then(|b| b.build()) {
...
"#)?
    ...
    match test_config.validate_ports().await {
        Ok(()) => println!("✓ Ports {} and {} are available", 
                          test_config.server.grpc_port, 
                          test_config.server.http_port),
