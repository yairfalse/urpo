...
async fn test_list_recent_traces() {
    ...
    for i in 1..traces.len() {
        assert!(traces[i-1].start_time >= traces[i].start_time, 
    ...
    let api_traces = storage.list_recent_traces(10, 
        Some(&ServiceName::new("api-gateway".to_string()).unwrap())).await.unwrap();
    assert_eq!(api_traces.len(), 5, "All traces have api-gateway service");
    
    let service_1_traces = storage.list_recent_traces(10, 
        Some(&ServiceName::new("service-1".to_string()).unwrap())).await.unwrap();
...
async fn test_get_slow_traces() {
    ...
    let slow_traces = storage.get_slow_traces(Duration::from_millis(500), 10).await.unwrap();
    assert_eq!(slow_traces.len(), 2, "Should have 2 slow traces");
    
    // Verify all returned traces are slow
    for trace in slow_traces {
        assert!(trace.duration >= Duration::from_millis(500), 
...
async fn test_search_traces() {
    ...
    for (i, &op) in operations.iter().enumerate() {
        ...
        storage.store_span(span_builder.build().unwrap()).await.unwrap();
    }
...
async fn test_span_hierarchy() {
    ...
    assert_eq!(spans.len(), 7, "Should have 7 spans total (1 root + 2 children + 4 grandchildren)");
    
    // Verify hierarchy
    let root_spans: Vec<_> = spans.iter().filter(|s| s.parent_span_id.is_none()).collect();
    assert_eq!(root_spans.len(), 1, "Should have exactly 1 root span");
    
    let child_spans: Vec<_> = spans.iter()
    ...
    for child in &child_spans {
        let grandchildren: Vec<_> = spans.iter()
...
async fn test_trace_aggregation_and_statistics() {
    ...
    let service_0_traces = storage.list_recent_traces(100, 
        Some(&ServiceName::new("service-0".to_string()).unwrap())).await.unwrap();
    ...
    for trace in &slow_traces {
        assert!(trace.duration >= slow_threshold, 
                "Trace {:?} duration {:?} should exceed threshold {:?}", 
                trace.trace_id, trace.duration, slow_threshold);
    }
}
...
async fn test_memory_pressure_and_cleanup() {
    ...
    assert!(memory_pressure > 0.0 && memory_pressure <= 1.0, 
            "Memory pressure should be between 0 and 1: {}", memory_pressure);
    ...
    for trace in recent_traces {
        let trace_num: usize = trace.trace_id.as_str()
            .strip_prefix("trace_")
...
async fn test_service_metrics_calculation() {
    ...
    for metrics in &[auth_metrics, api_metrics, db_metrics] {
        assert!(metrics.latency_p50 > Duration::from_millis(0), 
...
async fn test_concurrent_access_and_thread_safety() {
    ...
    assert_eq!(final_count, num_writers * spans_per_writer, 
               "Should have all spans: expected {}, got {}", 
               num_writers * spans_per_writer, final_count);
    ...
    for _ in 0..10 {
        ...
        let handle = task::spawn(async move {
            ...
            let slow_traces = storage_clone.get_slow_traces(Duration::from_millis(50), 100).await.unwrap();
...
async fn test_span_attributes_and_tags_filtering() {
    ...
    let test_data = vec![
        ("user-service", vec![("user.id", "12345"), ("user.email", "test@example.com")], vec!["production", "critical"]),
        ("payment-service", vec![("payment.amount", "99.99"), ("payment.currency", "USD")], vec!["production", "financial"]),
        ("notification-service", vec![("notification.type", "email"), ("notification.recipient", "admin")], vec!["staging", "low-priority"]),
        ("analytics-service", vec![("event.type", "page_view"), ("event.source", "mobile")], vec!["production", "analytics"]),
        ("auth-service", vec![("auth.method", "oauth"), ("auth.provider", "google")], vec!["production", "security"]),
    ];
    ...
    let user_op_search = storage.search_traces("user-service_operation", 10).await.unwrap();
    ...
    let not_found = storage.search_traces("nonexistent_value", 10).await.unwrap();
    ...
    assert!(first_span.attributes.contains_key("user.id"), "Span should have user.id attribute");
    assert!(first_span.attributes.contains_key("user.email"), "Span should have user.email attribute");
...
async fn test_trace_waterfall_visualization_data() {
    ...
    let mut children_map: std::collections::HashMap<Option<SpanId>, Vec<&Span>> = std::collections::HashMap::new();
    for span in &spans {
        children_map.entry(span.parent_span_id.clone()).or_insert_with(Vec::new).push(span);
    }
    ...
    let gateway_children = children_map.get(&Some(SpanId::new("gateway_root".to_string()).unwrap())).unwrap();
    assert_eq!(gateway_children.len(), 3, "Gateway should have 3 child spans");
    
    // Verify user service children (should have db and cache)
    let user_children = children_map.get(&Some(SpanId::new("user_fetch".to_string()).unwrap())).unwrap();
    ...
    assert_eq!(total_duration, Duration::from_millis(250), 
    ...
    assert!(user_start < auth_end && user_start > auth_start,
...
async fn test_edge_cases_and_error_conditions() {
    ...
    let empty_slow_traces = storage.get_slow_traces(Duration::from_millis(100), 10).await.unwrap();
    ...
    assert_eq!(single_trace_info[0].span_count, 1, "Single span trace should have span_count of 1");
    ...
    assert!(orphan_spans[0].parent_span_id.is_some(), "Orphan span should have parent_span_id set");
    ...
    assert!(long_trace_spans[0].operation_name.len() >= 1000, "Should preserve long operation name");
    ...
    assert_eq!(duplicate_spans[0].service_name.as_str(), "second-service", "Should have the second span's data");
    assert!(duplicate_spans[0].status.is_error(), "Should have the second span's error status");
    ...
    let filtered_traces = storage.list_recent_traces(10, Some(&non_existent_service)).await.unwrap();
    ...
    let recent_service_spans = storage.get_service_spans(
        &ServiceName::new("old-service".to_string()).unwrap(),
        recent_time
    ).await.unwrap();
    assert_eq!(recent_service_spans.len(), 0, "Should not return spans older than requested time");
    ...
    assert!(special_spans[0].attributes.get("special.chars").unwrap().contains("🎉"), 
