...
#[tokio::test]
async fn reality_check_performance_claims() {
    ...
    println!("Claims verified: {}/4", claims_verified);

    // The test should pass regardless - we just want to see the numbers
    assert!(claims_verified >= 0); // Always true, just to make it a test
}
