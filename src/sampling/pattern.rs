...
impl PatternDetector {
    ...
    pub async fn is_anomalous(&self, characteristics: &TraceCharacteristics) -> bool {
        ...
        // 1. Duration anomaly
        if self
            .is_duration_anomalous(characteristics.duration_ms)
            .await
        {
            return true;
        }
        ...
        if self
            .is_span_count_anomalous(characteristics.span_count)
            .await
        {
            return true;
        }
    ...
    async fn is_duration_anomalous(&self, duration_ms: Option<u64>) -> bool {
        ...
        let mean =
            patterns.iter().map(|p| p.total_duration_ms).sum::<u64>() / patterns.len() as u64;

        let variance = patterns
            .iter()
            .map(|p| {
    ...
    async fn is_span_count_anomalous(&self, span_count: usize) -> bool {
        ...
        let mean = patterns.iter().map(|p| p.span_count).sum::<usize>() / patterns.len();
    ...
    pub fn update_baseline(&self, service: ServiceName, latency_ms: u64) {
        let baseline = self
            .service_baselines
            .entry(service)
            .or_insert_with(|| LatencyBaseline {
