...
impl AdaptiveSampler {
    ...
    pub async fn set_sampling_rate(&self, rate: f64) {
        self.sampling_rate
            .store((rate * 10000.0) as u64, Ordering::Relaxed);
    }
...
impl DegradationController {
    ...
    pub async fn update_pressure(&self, memory: f64, cpu: f64, errors: f64) {
        ...
        self.memory_pressure
            .store((memory * 10000.0) as u64, Ordering::Relaxed);
        self.cpu_pressure
            .store((cpu * 10000.0) as u64, Ordering::Relaxed);
        self.error_rate
            .store((errors * 10000.0) as u64, Ordering::Relaxed);
    ...
    async fn change_mode(&self, new_mode: DegradationMode) {
        ...
        *self.last_change.lock().await = Instant::now();
        ...
        // Update sampling rate
        self.sampler
            .set_sampling_rate(new_mode.sampling_rate())
            .await;
...
fn count_enabled_features(features: &FeatureFlags) -> u32 {
    ...
    if features.trace_correlation {
        count += 1;
    }
    if features.service_discovery {
        count += 1;
    }
    if features.realtime_updates {
        count += 1;
    }
    if features.span_indexing {
        count += 1;
    }
    if features.histograms {
        count += 1;
    }
    if features.percentiles {
        count += 1;
    }
    if features.log_correlation {
        count += 1;
    }
    count
}
...
mod tests {
    ...
    async fn test_adaptive_sampler() {
        ...
        let service =
            ServiceName::new("test".to_string()).expect("Test service name should be valid");
