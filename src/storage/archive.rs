    ...
    pub fn parse_partition_key(&self, key: &str) -> Result<SystemTime> {
        match self {
            Self::Hourly => {
                ...
                .map_err(|e| {
                    UrpoError::Storage(format!("Invalid hourly partition key {}: {}", key, e))
                })?
                .and_utc();
            ...
            Self::Daily => {
                ...
                .map_err(|e| {
                    UrpoError::Storage(format!("Invalid daily partition key {}: {}", key, e))
                })?
                .and_utc();
                Ok(SystemTime::from(dt))
            }
            Self::Weekly => {
                ...
                if let Some(captures) = regex::Regex::new(r"(\d{4})W(\d{2})")
                    ...
                    .captures(key)
                {
                    let year: i32 = captures[1].parse().map_err(|e| {
                        UrpoError::Storage(format!("Invalid year in partition key {}: {}", key, e))
                    })?;
                    let week: u32 = captures[2].parse().map_err(|e| {
                        UrpoError::Storage(format!("Invalid week in partition key {}: {}", key, e))
                    })?;

                    let dt = chrono::NaiveDate::from_isoywd_opt(year, week, chrono::Weekday::Mon)
                        .ok_or_else(|| {
                            UrpoError::Storage(format!("Invalid ISO week date: {}W{}", year, week))
                        })?
                        .and_hms_opt(0, 0, 0)
...
impl ArchiveIndex {
    ...
    pub fn add_trace(
        &mut self,
        spans: &[Span],
        service_intern: &mut ServiceInterning,
    ) -> Result<()> {
        if spans.is_empty() {
            return Ok(());
        }
        ...
        let trace_id_hash = self.hash_trace_id(
            &spans
                .first()
        ...
        if has_error {
            let mut error_bitmap = if self.error_traces.is_empty() {
                ...
                RoaringBitmap::deserialize_from(&self.error_traces[..]).map_err(|e| {
                    UrpoError::Storage(format!("Failed to deserialize error bitmap: {}", e))
                })?
            };
            error_bitmap.insert(trace_id_hash);
            self.error_traces.clear();
            error_bitmap
                .serialize_into(&mut self.error_traces)
                .map_err(|e| {
                    UrpoError::Storage(format!("Failed to serialize error bitmap: {}", e))
                })?;
        }
        ...
        if is_slow {
            let mut slow_bitmap = if self.slow_traces.is_empty() {
                ...
                RoaringBitmap::deserialize_from(&self.slow_traces[..]).map_err(|e| {
                    UrpoError::Storage(format!("Failed to deserialize slow bitmap: {}", e))
                })?
            };
            slow_bitmap.insert(trace_id_hash);
            self.slow_traces.clear();
            slow_bitmap
                .serialize_into(&mut self.slow_traces)
                .map_err(|e| {
                    UrpoError::Storage(format!("Failed to serialize slow bitmap: {}", e))
                })?;
        }
        ...
        for service_name in services {
            ...
            self.service_names
                .insert(service_id, service_name.to_string());

            // Update service traces bitmap
            let mut service_bitmap =
                if let Some(existing_data) = self.service_traces.get(&service_id) {
                    RoaringBitmap::deserialize_from(&existing_data[..]).map_err(|e| {
                        UrpoError::Storage(format!("Failed to deserialize service bitmap: {}", e))
                    })?
                } else {
                    RoaringBitmap::new()
                };
            ...
            service_bitmap
                ...
                .map_err(|e| {
                    UrpoError::Storage(format!("Failed to serialize service bitmap: {}", e))
                })?;
    ...
    pub fn get_error_trace_ids(&self) -> Result<Vec<u32>> {
        ...
        let bitmap = RoaringBitmap::deserialize_from(&self.error_traces[..]).map_err(|e| {
            UrpoError::Storage(format!("Failed to deserialize error bitmap: {}", e))
        })?;
...
impl ArchiveWriter {
    ...
    pub fn flush_current_partition(&mut self) -> Result<()> {
        ...
        tracing::info!(
            "Flushing archive partition: {} ({} traces)",
            partition_key,
            self.current_traces.len()
        );
        ...
        let compression_ratio =
            compressed_traces.len() as f32 / (traces.len() * std::mem::size_of::<Span>()) as f32;
    ...
    pub fn get_archive_stats(&self) -> ArchiveStats {
        ...
        if stats.total_partitions > 0 {
            stats.avg_compression_ratio =
                indices.values().map(|i| i.compression_ratio).sum::<f32>() / indices.len() as f32;
        }
...
mod tests {
    ...
    fn test_archive_index_operations() {
        ...
        let trace2 = vec![create_test_span("trace2", "service-a", 100)];
